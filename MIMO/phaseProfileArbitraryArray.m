...
function [rangeDataPeak,xM,yM] = phaseProfileArbitraryArray(sarDataIn,iParams,fParams)
...
if iParams.CSAR
    ...
    elseif ndims(sarDataIn) == 3 && min(size(sarDataIn) == [iParams.nVerMeasurement,iParams.nUsefulHorMeasurement,fParams.adcSample])
        ...
        % sarDataIn = s(y,x,k)
        sarDataIn = permute(sarDataIn,[3,1,2]);
    elseif ndims(sarDataIn) == 3 && min(size(sarDataIn) == [iParams.nVerMeasurement*iParams.nVx,iParams.nUsefulHorMeasurement,fParams.adcSample])
        % MIMO-SAR 2D
        % sarDataIn = s(nTx*nRx*y,x,k)
        sarDataIn = permute(sarDataIn,[3,1,2]);
    else
