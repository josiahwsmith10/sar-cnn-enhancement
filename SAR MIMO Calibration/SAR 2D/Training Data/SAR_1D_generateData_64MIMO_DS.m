...
iParams.locTx_m =  [0       0
                    ...
                    0       20
                    0       24
                    0       28]*lambda_m + [iParams.TxRxOffsetx_m,iParams.TxRxOffsety_m];
...
% Create iParams
%-------------------------------------------------------------------------%
iParams.nVerMeasurement = 8;
iParams.z0_mm = 500;
iParams.nFFT = 2^(ceil(log2(iParams.nVerMeasurement*iParams.nVx)));
...
%% Directory for Files
%-------------------------------------------------------------------------% 
ds_path = "D:/Research/SAR MIMO Calibration/SAR 2D/64 MIMO Huge Dataset/";
...
%     max(Image(:))
    ...
    for indTarget = 1:numTarget
        ...
        Output = Output + exp(1j*2*k.*R);
    end
    
    Input = phaseCorrectionArbitraryArray(Input,iParams,fParams);
...
sarImageInput = SAR_1D_reconstructImage_2D_RMA_MIMO(complex(Input(:,:,1),Input(:,:,2)),iParams,fParams,p);