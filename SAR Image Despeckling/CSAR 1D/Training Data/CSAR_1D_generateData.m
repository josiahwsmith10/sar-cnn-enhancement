...
nAngMeasurement = iParams.nAngMeasurement;
...
for indSample = 1:numSample
    ...
    % Input and Output
    csarData = gpuArray(single(complex(zeros(nAngMeasurement, fParams.adcSample))));
    ...
    for indTarget = 1:numTarget
        ...
        Output(:,:,indSample) = Output(:,:,indSample) + abs(amp_xz(indTarget))*exp( - (X - coord_x_m(indTarget)).^2/xz_resolution^2 - (Z - coord_z_m(indTarget)).^2/xz_resolution^2);
    end
    
    csarDataUpsampled = interpn(theta_rad,k,csarData,theta_radUpsample,kUpsample,"linear",0);
    csarDataUpsampledFFT = fft(csarDataUpsampled,[],1);
    azimuthFiltered = conj(ifft(csarDataUpsampledFFT .* conj(azimuthFilterFFT),[],1));
    
    csarImageFFT = interpn(theta_radUpsample,kUpsample,azimuthFiltered,theta_radU,kU,"linear",0);
...
Input = Input./max(Input,[],[1,2]);
Output = Output./max(Output,[],[1,2]);

%% Save the Training Data

save("trainingSet","Input","Output","numTargetMax","numSample")