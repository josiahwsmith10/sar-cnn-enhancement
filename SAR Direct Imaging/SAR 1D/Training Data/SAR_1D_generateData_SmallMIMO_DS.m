...
%% Add Paths
%-------------------------------------------------------------------------%
addpath(genpath("../../../../sar-simulation-jws"),genpath("../../"))
...
iParams.locTx_m =  [0       0
                    0       2]*lambda_m + [iParams.TxRxOffsetx_m,iParams.TxRxOffsety_m];
...
%% Directory for Files
%-------------------------------------------------------------------------% 
ds_path = "D:/Research/SAR Direct Imaging/SAR 1D/Small MIMO/";
...
k = 2*pi*f/c;
k = reshape(k,1,[]);
kUpsample = reshape(linspace(min(k),max(k),512),1,[]);
clear f f0
...
%     Output = Input;
    ...
    coord_y_m = p.yT(end/2);
    coord_z_m = p.zT(end/2);
    amp_yz = 1.5 + 0.1*rand(numTarget,1);
    
    [Y,Z] = ndgrid(p.yT,p.zT);
    Output = reshape(amp_yz,1,1,[]).*exp( - (Y - reshape(coord_y_m,1,1,[])).^2/0.001^2 - (Z - reshape(coord_z_m,1,1,[])).^2/0.001^2);
    Output = single(sum(Output,3));
...
%         Output = Output + R.^(-2) .* exp(1j*2*k.*R);
    ...
    Input = interpn(yV_m,k,Input,yV_m,kUpsample,"linear",0);
...
%-------------------------------------------------------------------------%
% phaseProfileArbitraryArray2D(complex(Input(:,:,1),Input(:,:,2)),iParams,fParams);

sarImageInput = SAR_1D_reconstructImage_2D_RMA_MIMO(complex(Input(:,1:8:end,1),Input(:,1:8:end,2)),iParams,fParams,p);